...
import { calLUDecomposition } from '../containers/calculator';
...
class LUDecomposition extends React.Component{
    ...
    onClickCalculate = (e) =>{
        ...
        try{
            ...
            let { data } = calLUDecomposition(this.state.n, this.state.matrixA, this.state.matrixB)
            let arr = []
            data.map((x, i) => {
                arr.push({
                    key: i+1,
    ...
    componentDidMount() {
        this.props.setKeys(['10'])
    }

    render(){
        return(
            <div className="content-layout-background">
                <h1 className="content-header">LU Decomposition Method</h1>
